...
#### Sample collection and environmental conditions
  ...
  A total of 84 tissue samples from scleractinian and milleporine corals were collected from seven sites around St. John, US Virgin Islands, USA (Fig. 1), between August 7th and 9th, 2012. Samples were collected on snorkel from 3-7m depth, and all colonies were selected haphazardly in upward facing locations exposed to ambient irradiance at the sampling depth. Tissue biopsies (~5 mm diameter and ~5 mm depth) were removed from coral colonies with sterile metal borers, and the small holes were plugged with non-toxic modeling clay. 

  Broad-scale differences in the hydrographic regimes at points representative of the north and south shores of St. John (North: 18.368141°N, 64.711967°W; South: 18.315465°N, 64.726840°W) were evaluated from remote sensing tools to determine sea surface temperature (°C), surface chlorophyll concentrations (mg chl a m^-3^), and wave exposure (J m^-3^). These features were selected as they corresponded to anecdotal observations of differences between the water masses on both shores, and reflected conditions that strongly affect coral performance and *Symbiodinium* biology (i.e., SST [@Coles:1977p7350] and wave regime [@Atkinson:1994un]), and reflect nutrients and planktonic food resources that play important roles in coral nutrition [@Houlbreque:2009gh]. SST was evaluated from nighttime records, and chlorophyll from ocean colour data, both of which were obtained at 1 km spatial resolution from the MODIS sensor aboard the Aqua satellite for each month of the year, averaged over the period 2003-2010. We used as input the monthly climatologies available at the IMaRS website (http://imars.usf.edu/modis); if data were not available at the chosen coordinates, values from the nearest available pixel were used. Boxplots were generated from the 12 monthly climatological values obtained for each response (Fig. 1).

  Wind-driven wave exposure for a given site is dependent on the wind patterns and the configuration of the coastline, which defines the fetch, or the length of water over which a given wind has blown to generate waves. To calculate wave exposure, wind speed and direction data at each location were acquired from the QuickSCAT (NASA) satellite scatterometer from 1999 to 2008 at 25 km spatial resolution [@Hoffman:2005dz]. Coastline data were obtained from the Global Self-consistent, Hierarchical, High-resolution, Shoreline (GSHHS v 2.2) database, which provides global coastline at 1:250,000 scale [@Wessel:1996wk]. From these data, wave exposure was calculated using the methods based on wave theory (after Chollett et al. 2012) for 32 fetch directions and coastline data at full resolution. Mean wave exposure at each station was calculated in R using the packages maptools [@maptoolsToolsfor:_dYm01UK], raster [@rasterGeographicD:524kz2M0], rgeos [@rgeosInterfaceto:8WVz_nNk], and sp [@Bivand:2013ux].
...
#### Sample collection and environmental data
  ...
  A total of 84 coral samples representing ten different host species were collected across two sites on the north shore and five sites on the south shore of St. John (Fig. 1). The sequence data generated from these samples comprised `r system("grep -c '>' data/fasta/combined_seqs_trimmed.fasta")` sequences after merging paired reads, removing chimeric sequences, and trimming primers. After clustering, OTUs with <`r n` sequences and samples with <`r sn` sequences were excluded, leaving `r nsamples(phy97.f)` samples for downstream analysis.
  
  The environmental conditions broadly characterizing the north and south shores are presented in Figure 1. From 2003 to 2010, the north shore of St. John was characterized by slightly lower sea surface temperatures, higher chlorophyll a concentrations, and lower wave exposure, relative to the south shore.
...
plot(NA, xlim=c(3,4.2), ylim=c(0.5,2.5), xlab="Wave exposure (J m-3)", ylab="", bty="n", yaxt="n")
points(c(we$N, we$S), c(2,1), pch=19)
text(par("usr")[1], c(1,2), c("S","N"), pos=2, offset=1)
points(rep(par("usr")[1]-strwidth("c.m"),2), c(1,2), pch=1, cex=2.5)
...
```{r Fig3, fig.height=6, fig.width=4, fig.align="center", fig.cap="Figure 3. *Symbiodinium* community composition of each sample. Samples are plotted as horizontal bars, sorted by species and shore of collection (north vs. south). Individual segments of each bar represent distinct 97% within-sample OTUs, colored by *Symbiodinium* clade identity."}
...
#### *Symbiodinium* metacommunity ecology in St. John
  ...
  While environmental data revealed significant biophysical differences between the north and south shores of the island (e.g., wave exposure, temperature, and chlorophyll A), there were no significant differences in symbiont community composition in the corals sampled. This may partly be due to low statistical power associated with small sample sizes, since two species showed clear trends: *S. siderea* and *P. furcata* were dominated more frequently by *Symbiodinium* in clades D and A, respectively, on the south shore, compared to the north shore (Fig. 2). Since these symbionts are typically associated with warmer and more variable temperature regimes [e.g., @Oliver:2011cr] and high-light habitats [@Byler], it is consistent with expectation that they were more prevalent on the south shore, where temperatures were slightly higher and chlorophyll levels were lower, suggesting light penetration is likely greater. However, the lack of differentiation in the symbiont communities of most host species suggests their structure is not sensitive to the level of environmental variation between shores.
  
  When the associations between *Symbiodinium* types and coral species were visualized by network analysis, features of both symbiont and host ecology emerge. First, ITS2 type B1 was "abundant" (i.e. >1%) in individuals of seven out of ten coral species, and "dominant" (i.e. >50%) in five out of ten, suggesting it is a more generalist symbiont lineage. However, analysis of more variable chloroplast and microsatellite loci has revealed that ITS2 type B1 in the Caribbean is comprised of numerous specialized lineages that demonstrate specific associations with different coral host species [@Santos:2004p4044; @Finney:2010p8324; @Parkinson:2015gt]. Likewise, the apparent generalists *Symbiodinium* C3 (which dominated colonies of both *S. siderea* and *M. cavernosa*) and *Symbiodinium* A4 (which dominated both *P. astreoides* and *P. furcata*) may also be comprised of multiple specialized lineages [@Thornhill:2013dx], which will require evaluation with population genetic markers.
