...
read.blast <- function(file) {
  b <- read.table(file, stringsAsFactors=FALSE,
                  col.names=c("otu", "pct.id", "length", "mismatches", "gapopen", "q.start", "q.end", 
                              "s.start", "s.end", "e.val", "bit.score", "hit"))
  b <- b[order(b$otu),]
  return(b)
}
b <- read.blast("data/otus_97/blast_tophits.tsv")
...
nw <- read.nw("data/otus_97/nw_tophits.tsv")

tax.import <- function(bfp, nwfp) {
  b <- read.table(bfp, col.names=blast.res.colnames, stringsAsFactors=FALSE)
  b <- b[order(b$otu),]
  ag <- aggregate(data.frame(BLAST.hit=b$BLAST.hit), by=list(otu=b$otu), FUN=function(x) paste0(x, collapse=";"))
  b2 <- b[!duplicated(b$otu), ]
  b2$BLAST.hit <- ag$BLAST.hit
  nw <- read.table(nwfp, stringsAsFactors=FALSE)
  m <- merge(b2, nw, by="otu")
  rownames(m) <- m$otu
  m$Clade <- str_extract(m$nw.hits, pattern="^.")
  return(m)
}

tax97 <- tax.import(bfp="data/otus_97/blast_tophits_withties.txt", nwfp="data/otus_97/nw_tophits.tsv")
tax100 <- tax.import(bfp="data/otus_100/blast_tophits.txt", nwfp="data/otus_100/nw_tophits.tsv")
tax97bs <- tax.import(bfp="data/otus_97_bysample/blast_tophits.txt", nwfp="data/otus_97_bysample/nw_tophits.tsv")
tax97bsp <- tax.import(bfp="data/otus_97_byspecies/blast_tophits.txt", nwfp="data/otus_97_byspecies/nw_tophits.tsv")
...
collapse.idents <- function(df) {
  within(df, {
    for (i in 1:nrow(ident)) {
      BLAST.hit <- gsub(ident[i,1], ident[i,2], BLAST.hit)
      nw.hits <- gsub(ident[i,1], ident[i,2], nw.hits)
      nw.hit1 <- gsub(ident[i,1], ident[i,2], nw.hit1)
    }
  })
}
...
# Get subtypes without accession numbers
get.st <- function(df) {
  within(df, {
    bl.st <- gsub(BLAST.hit, pattern="_.*$", replacement="")
    bl.st <- ifelse(nw.sim==100, bl.st, paste("'", bl.st, "'", sep=""))
    nw.st <- gsub(nw.hits, pattern="_[A-Z]{2}[0-9]{6}", replacement="")
    nw.st <- gsub(nw.st, pattern="_multiple", replacement="")
    nw.st <- ifelse(nw.sim==100, nw.st, paste("'", nw.st, "'", sep=""))
    Subtype <- nw.st
  })
}
...
# Build function to BLAST all sequences with less than 90% similarity to Symbiodinium to the NCBI nr database, and get the top hit. If the top hit does not contain the string "Symbiodinium", then this sequence is assumed to not be Symbiodinium.
is.it.symbio <- function(df, repset) {
  poor.otus <- df[which(df$nw.sim < 90), "otu"]  # Identify OTUs with <90% similarity to Symbiodinium
  sapply(poor.otus, function(otu) {  # Get sequences for these OTUs to BLAST to NCBI
    system(paste("awk '/", otu, " /{print;getline;print}' ", repset, " >> temp.fasta", sep=""))
  })
  blast.res <- system("blastn -db nr -remote -query temp.fasta -outfmt '6 qseqid qcovs stitle sseqid' -max_target_seqs 1 | sort -u", intern=TRUE)  # Blast sequences to NCBI
  blast.res <- data.frame(do.call(rbind, lapply(strsplit(blast.res, split="\t"), "[", c(1,3))),
                          stringsAsFactors=FALSE)
  blast.res$sym <- str_detect(blast.res[, 2], "Symbiodinium")  # See if Symbiodinium is in BLAST result
  df2 <- setNames(merge(poor.otus, blast.res, by=1, all=T), nm=c("otu", "hit", "sym"))
  df2[which(is.na(df2$hit)), c("hit", "sym")] <- c("No hit", FALSE)
  system("rm temp.fasta")  # Cleanup temp file
  return(df2)
}

tax97.blasts <- is.it.symbio(tax97, "data/otus_97/97_otus_rep_set.fasta")
tax97.notsym <- as.character(tax97.blasts[which(tax97.blasts$sym==FALSE), "otu"])

tax97bs.blasts <- is.it.symbio(tax97bs, "data/otus_97_bysample/all_rep_set_rep_set.fasta")
tax97bs.notsym <- as.character(tax97bs.blasts[which(tax97bs.blasts$sym==FALSE), "otu"])

tax97bsp.blasts <- is.it.symbio(tax97bsp, "data/otus_97_byspecies/all_rep_set_rep_set.fasta")
tax97bsp.notsym <- as.character(tax97bsp.blasts[which(tax97bsp.blasts$sym==FALSE), "otu"])

tax100.blasts <- is.it.symbio(tax100, "data/otus_100/100_otus_rep_set.fasta")
tax100.notsym <- as.character(tax100.blasts[which(tax100.blasts$sym==FALSE), "otu"])

# Remove OTUs that are not Symbiodinium
phy97.f <- prune_taxa(!taxa_names(phy97) %in% tax97.notsym, phy97)
phy97bs.f <- prune_taxa(!taxa_names(phy97bs) %in% tax97bs.notsym, phy97bs)
phy97bsp.f <- prune_taxa(!taxa_names(phy97bsp) %in% tax97bsp.notsym, phy97bsp)
phy100.f <- prune_taxa(!taxa_names(phy100) %in% tax100.notsym, phy100)
...
composition(phy97bs.f.p, col=taxcolors[factor(data.frame(tax_table(phy97bs.f.p))[order(data.frame(tax_table(phy97bs.f.p))$nw.st), ]$Clade, levels=c("A","B","C","D","F","G"))], legend=T)
...
V(net)$label <- ifelse(is.na(V(net)$Clade), names(V(net)),  
                       unlist(lapply(strsplit(V(net)$nw.st, split="_"), "[", 1)))
...
nodemapnew[grep("^D", V(net)$nw.st)] <- min(grep("^D", V(net)$nw.st))
