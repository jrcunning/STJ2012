...
shorestats <- data.frame(matrix(dimnames=list(levels(data.frame(sample_data(phy97.f.t))$Species), 
                                              c("Species", "n", "overall", "within", 
                                                "between", "R2", "bd", "p")), ncol=8, nrow=10))
for (i in 1:nlevels(data.frame(sample_data(phy97.f.t))$Species)) {
  sp <- levels(data.frame(sample_data(phy97.f.t))$Species)[i]
  shorestats$Species[i] <- sp
  phy97.f.t.species <- subset_samples(phy97.f.t, Species==sp)  # Set temp phyloseq object
  shorestats$n[i] <- nsamples(phy97.f.t.species)
  md <- meandist(phyloseq::distance(phy97.f.t.species, "bray"), 
                 sample_data(phy97.f.t.species)$Shore)
  ...
  if (nlevels(as(sample_data(phy97.f.t.species), "data.frame")$Shore) > 1) {
    set.seed(70235)
    permanova <- adonis(phyloseq::distance(phy97.f.t.species, "bray") ~ Shore, 
                        data=as(sample_data(phy97.f.t.species), "data.frame"), permutations=999)
    ...
    bd <- betadisper(phyloseq::distance(phy97.f.t.species, "bray"),
                     group=as(sample_data(phy97.f.t.species), "data.frame")$Shore)
