...
sapply(list.files(path="R", pattern="*[^w].R", full.names=TRUE), source, .GlobalEnv)
...
blast.res.colnames <- c("otu", "BLAST.hit", "pct.id", "B.length", "B.mismatches", "B.gapopen", 
                        "q.start", "q.end", "s.start", "s.end", "e.val", "bit.score")
tax.import <- function(b, nw) {
  m <- merge(read.table(b, col.names=blast.res.colnames, stringsAsFactors=FALSE),
             read.table(nw, stringsAsFactors=FALSE), by="otu")
  rownames(m) <- m$otu
  m$Clade <- str_extract(m$nw.hits, pattern="^.")
  return(m)
}

tax97 <- tax.import(b="data/otus_97/blast_tophits.txt", nw="data/otus_97/nw_tophits.tsv")
tax100 <- tax.import(b="data/otus_100/blast_tophits.txt", nw="data/otus_100/nw_tophits.tsv")
tax97bs <- tax.import(b="data/otus_97_bysample/blast_tophits.txt", nw="data/otus_97_bysample/nw_tophits.tsv")
tax97bsp <- tax.import(b="data/otus_97_byspecies/blast_tophits.txt", nw="data/otus_97_byspecies/nw_tophits.tsv")
...
# Replace any sequence in taxonomy assignment that is a member of a group of identical sequences with the name of the group
